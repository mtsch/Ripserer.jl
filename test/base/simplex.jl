...
@testset "Simplex" begin
    ...
    @testset "AbstractSimplex interface" begin
        for D in (0, 2, 4), T in (Float64, Float32, Int), I in (Int32, Int64, Int128)
        ...
        @testset "type stability" begin
            for dim in 1:10
                ...
                @static if VERSION ≥ v"1.1.0"
                    @test begin
                        @inferred Union{
                            Nothing,Tuple{Simplex{dim + 1,Int,Int},Tuple{Int,Int}}
                        } iterate(cobiter)
                        true
                    end
                    @test begin
    ...
    @testset "Boundary" begin
        ...
        @testset "type stability" begin
            for dim in 1:10
                simplex = Simplex{dim}(10, 1)
                @test begin
                    @inferred boundary(FakeFiltration(), simplex)
                    true
                end
                biter = boundary(FakeFiltration(), simplex)
                @static if VERSION ≥ v"1.1.0"
                    @test begin
                        @inferred Union{Nothing,Tuple{Simplex{dim - 1,Int,Int},Int}} iterate(
                            biter
                        )
                        true
                    end
